...
class Sam(sprite.Sprite):
    ...
    def __init__(self, game_env, source, target, flip):
        ...
        self.surf = image.load(self.__game_env.static.sam).convert()                # loading sam image
    ...
    def update(self):
        ...
        if self.rect.right < 0 or self.rect.left > self.__game_env.static.screen_width or self.rect.bottom < 0:
            self.kill()
