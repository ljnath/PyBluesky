...
class GameInputText(Text):
    """ GameInputText class extended from Text class.
        It creates the game input menu sprite
    """
    def __init__(self, game_env): 
        Text.__init__(self, game_env, size=48)                                                                          # initilizing parent class with default text color as red
        ...
        keybrd_surf = self.font.render(' Keyboard', 1, game_env.constants.text_selection_color)                         # creating surface with Keyboard text when highlighted
        ...
        mouse_surf = self.font.render(' Mouse', 1, game_env.constants.text_selection_color)                             # creating surface with mouse text when highlighted
    ...
    def update(self, pressed_keys):
        if pressed_keys[self.__game_env.K_UP]:                                                                          # checking if user has pressed UP
            self.__game_env.variables.game_input = InputMode.KEYBOARD                                                   # setting game input mode as keyboard
            ...
            self.__game_env.variables.game_input = InputMode.MOUSE                                                      # setting game input mode as mouse
            self.__highlightMouse()                                                                                     # calling method to highlight mouse
        self.rect = self.surf.get_rect(center=(self.__game_env.constants.screen_width/2, self.__game_env.constants.screen_height/2))        # creating default rect and setting its position center
