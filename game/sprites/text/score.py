...
class ScoreText(Text):
    ...
    def update(self, level, playtime, score):
        self.surf = self.font.render("LEVEL {} TIME {} AMMO {} SCORE {}".format(str(level).zfill(2), str(playtime).zfill(5), str(self.__game_env.variables.ammo).zfill(3),str(score).zfill(5)), 1, self.color)   # updating scoreboard score and time
